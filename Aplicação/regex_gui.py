...
class Regex(Hideble):
	...
	def __init__(self, regex, master=None, topLevel=None, name="new-regex"):
		...
		self.concat.bind("<ButtonRelease-1>", lambda event, command=self.concatWord, label="Concatenar": self.createNewRegexFrame(command, label, "Nova Regex"))
		...
		self.union["width"] = 20
		self.union.bind("<ButtonRelease-1>", lambda event, command=self.unionWord, label="Unir": self.createNewRegexFrame(command, label, "Nova Regex"))
	...
	def file_save(self, event):
		f = asksaveasfilename(defaultextension=".er", filetypes=[('Arquivos de Expressão Regular', '.er'), ('Todos os arquivos', '.*')])
		if not f:
			return
		self.regex.save(f)
