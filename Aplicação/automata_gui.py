...
class Automata(Hideble):
	...
	def __init__(self, automata, master=None, topLevel=None, name="new-automata"):
		...
		self.addState.bind("<ButtonRelease-1>", lambda event, command=self.addNewState, label="Adicionar": self.createStateSelect(command, label, "Nome do Estado"))
		...
		self.removeState.bind("<ButtonRelease-1>", lambda event, command=self.removeOldState, label="Remover": self.createStateSelect(command, label, "Nome do Estado"))
		...
		self.addFinalState.bind("<ButtonRelease-1>", lambda event, command=self.addAcceptState, label="Adicionar": self.createStateSelect(command, label, "Nome do Estado"))
		...
		self.removeFinalState.bind("<ButtonRelease-1>", lambda event, command=self.removeAcceptState, label="Remover": self.createStateSelect(command, label, "Nome do Estado"))
		...
		self.setInitial.bind("<ButtonRelease-1>", lambda event, command=self.alterInitialState, label="Alterar": self.createStateSelect(command, label, "Nome do Estado"))
		...
		self.check["width"] = 20
		self.check.bind("<ButtonRelease-1>", lambda event, command=self.checkString, label="Checkar": self.createStateSelect(command, label, "String"))
		self.check.pack()
		...
		self.determinize.bind("<ButtonRelease-1>", lambda event, command=self.determinizeAutomata, label="Sim": createConfimationBox(self.master, command, "Ao continuar, voce ir transformar o AFND Atual em AFD. Deseja continuar?"))
		...
		self.convertRG.bind("<ButtonRelease-1>", lambda event, command=self.convertToGrammar, label="Sim": createConfimationBox(self.master, command, "Ao continuar, voce ir transformar o AFD Atual em GR. Deseja continuar?"))
