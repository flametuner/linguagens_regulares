...
class GrammarGui(Hideble):
    ...
    def __init__(self, grammar, master=None, topLevel=None, name="new-grammar"):
        ...
        self.convertAFND.bind("<ButtonRelease-1>",
                              lambda event, command=self.convertToAFND, label="Sim": createConfimationBox(self.master,
                                                                                                          command,
                                                                                                          "Ao continuar, voce ir transformar a GR Atual em AFND. Deseja continuar?"))
    ...
    def file_save(self, event):
        f = asksaveasfilename(defaultextension=".gr",
                              filetypes=[('Arquivos de Gramatica Regular', '.gr'), ('Todos os arquivos', '.*')])
        if not f:
            return
        self.grammar.save(f)
